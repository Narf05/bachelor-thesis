...
2. Call get_or_process() on each file to tokenize and cache in processed.db
...
# Make sure code/ is on the path when running this file directly.
...
def process_speaker_dir(
    ...
    *,
    ...
    pending = [f for f in txt_files if f"{corpus_source}__{f.stem}" not in done]
...
def sync_cache_to_speaker_dir(corpus_source: str, speaker_dir: Path) -> None:
    ...
    valid_names = {
        f"{corpus_source}__{txt_path.stem}" for txt_path in speaker_dir.glob("*.txt")
    }
...
def main():
    ...
    parser.add_argument(
        "--fillers",
        action="store_true",
        help="Strip filler words (um, uh, ...) during tokenization.",
    )
    parser.add_argument(
        "--min-words",
        type=_parse_min_words,
        default=400,
