...
\paragraph{Interpretation.}
The curve rises quickly at small sample sizes and then begins to flatten, but it has not fully saturated at the observed sample size. And it has a log-like shape. From there it follows that that additional sampling would still be expected to reveal new word types at a slower rate. The extrapolated part should be read as a model-based projection rather than as an observed quantity.
...
\paragraph{Interpretation.}
This table shows that there is an imbalance in the number of tokens per language. France has the highest one, then followed by english and then German. Even though English has a higher number of tokens, its mean coverage is smaller then the one of the German one by approximately 6 percent. Therefore, later comparisons should use normalized quantities, such as $\hat{S}/S_{\mathrm{obs}}$ and coverage, instead of relying only on raw estimated richness.
...
\paragraph{Interpretation.}
The French language corpora has the highest sample coverage, this implies that it has the highest completeness. And from that it follows it has the lowest amount of word appearing only once.  
...
\paragraph{Interpretation.} 
The singleton proportions support the coverage interpretation. French has the smallest singleton share relative to observed vocabulary, while German has the largest. This helps explain why German requires more extrapolation despite having a smaller total corpus.
Higher coverage means that a smaller fraction of the next sampled tokens is expected to belong to previously unseen word types. Thus, lower coverage corpora require stronger extrapolation, while higher coverage corpora are closer to being complete with respect to their observed sampling frame.
...
\paragraph{Interpretation.}
When fitting finite-k monote models, the data sticks with small/moderate k instead of pushing toward very large k. And at no point did the model kep choosing the maximum tested k. This means that the CM behavior is only an additional model for comparison for this data and will not necesseraly be chosen as the best one.  Therefore, the completely monotone model is useful as a limiting comparison, but the BIC selection does not by itself prove that complete monotonicity is required.
...
\paragraph{Interpretation.}
We can clearly see that all curves are not close to saturation and grow with the sample size. On top of that all of them aim towards the same expected number. The representative-unit comparison should not be interpreted as proving that the languages have the same vocabulary richness, because each panel is based on one selected unit and the corpora differ strongly in size and composition.
...
The important columns here are the median observed vocabulary, the median estimated total richness, the median unseen component $\hat{f}_0$, and the normalized ratio $\hat{S}/S_{\mathrm{obs}}$. The raw estimates are highest for French, but the normalized ratios are much closer across languages, which shows why raw $\hat{S}$ values should not be compared without considering sample size and coverage.
...
\paragraph{Interpretation.}
When comparing the ratios and not the raw estimates we can see how it affects the results. The French corpora doesnt score as high anymore. This is mainly due to the fact that the number of estimated words doesnt grow linearly with the number of tokens but rather logarithmically. The ratio plot is therefore more useful for comparing estimator behavior than the raw $\hat{S}$ plot. It shows that the amount of extrapolation relative to observed vocabulary is broadly similar across languages for a given estimator, while estimator choice still has a large effect.
