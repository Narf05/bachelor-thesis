...
\paragraph{Interpretation (Table~\ref{tab:results_shakespeare_estimator_summary}).}
The classical lower-bound estimators give in total between 35,000 and 41,500 types, while ACE gives a substantially larger estimate of about 70,900 types. This difference reflects the fact that ACE includes an explicit heterogeneity correction for the rare part of the frequency spectrum. Breakaway is not reported for the full Shakespeare corpus because the fitted extrapolation parameter was negative, making the estimated number of unseen types undefined.
...
\paragraph{Interpretation (Figure~\ref{fig:results_rarefaction_extrapolation_shakespeare}).}
The curve rises steeply at small sample sizes and then begins to flatten, following a roughly logarithmic shape, but has not fully saturated at the observed sample size. Additional sampling would therefore still be expected to reveal new word types, albeit at a diminishing rate. The extrapolated portion should be read as a model-based projection rather than as an observed quantity.
...
\paragraph{Interpretation (Table~\ref{tab:results_language_corpus_overview}).}
This table shows that there is an imbalance in the number of tokens per language. French has the highest, followed by English and then German. Even though English has a higher number of tokens, its mean coverage is smaller than that of German by approximately 6 percent. Therefore, later comparisons should use normalized quantities, such as $\hat{S}/S_{\mathrm{obs}}$ and coverage, instead of relying only on raw estimated richness.
...
\paragraph{Interpretation (Table~\ref{tab:results_corpus_overview}).}
At the corpus level, CLAPI dominates the total amount of data, while IMSDb contributes most of the English units. The table also shows that some corpora contain very small units, which can make richness estimation unstable. Shakespeare is included only for context and should not be interpreted as part of the spoken-language comparison.
...
\paragraph{Interpretation (Figure~\ref{fig:results_coverage_by_language}).}
French has the highest sample coverage across all three languages, indicating that its observed sample is the most complete relative to the estimated total vocabulary. Consequently, French also has the lowest proportion of word types appearing only once, since high coverage is directly linked to a smaller singleton fraction.
...
\paragraph{Interpretation (Table~\ref{tab:results_language_frequency_spectrum_summary}).}
The singleton proportions reinforce the coverage picture. French has the smallest singleton share relative to observed vocabulary ($f_1/S_{\mathrm{obs}} = 0.34$), while German has the largest ($0.56$). A higher singleton fraction means that a larger share of the next sampled tokens is expected to belong to previously unseen types, which implies lower coverage and stronger required extrapolation. This explains why German requires the most extrapolation despite having the smallest total corpus.
...
\paragraph{Interpretation (Table~\ref{tab:results_per_language_estimator_summary} and Figure~\ref{fig:results_estimator_boxplots_by_language}).}
Breakaway fails quite often because it does not have enough consecutive nonzero frequency counts for a stable ratio-regression fit; when the fitted parameter is negative it is undefined. When comparing normalized estimates across estimators, the ratios are broadly similar across languages, as also visible in Figure~\ref{fig:results_estimator_boxplots_by_language}. Across all three languages, Jackknife1 is the most conservative estimator, Chao1 and Jackknife2 are intermediate, ACE is more aggressive, and Breakaway has the largest median ratio among the successful fits. The normalized ratios are fairly similar across languages: for example, Chao1 gives median ratios around 2.1--2.35, ACE around 3.8--4.1, and Breakaway around 4.9--5.3. This suggests that estimator choice affects the result more strongly than language alone.
...
\paragraph{Interpretation (Table~\ref{tab:results_cm_kmonotone_summary}).}
Across all languages and corpora, the AIC-selected $k^\ast$ is small to moderate and never reaches the maximum tested value. This means the data do not push toward the completely monotone limit on their own: the finite-$k$ model is adequate and complete monotonicity is not required by the selection criterion. The completely monotone model is therefore useful as a theoretical benchmark, but should not be treated as the uniquely correct choice for these data.
...
\paragraph{Interpretation (Figure~\ref{fig:results_cm_vs_kmonotone_scatter}).}
The AIC-selected finite-$k^\ast$ monotone estimates and the completely monotone estimates are in near-perfect agreement: almost all points lie on the diagonal, meaning both approaches yield essentially the same estimated richness. Strong deviation from the diagonal would have indicated that the choice between finite-$k$ and complete monotonicity materially affects the estimates, but that is not the case here. In practice, the modelling choice has more impact on the theoretical interpretation than on the numerical result.
...
\paragraph{Interpretation (Table~\ref{tab:results_cross_language_estimator_summary}).}
Unlike Table~\ref{tab:results_per_language_estimator_summary}, which reported mean and IQR of $\hat{S}$ together with failure counts, this table focuses on the median observed vocabulary, the median estimated total richness, the median unseen component $\hat{f}_0$, and the normalized ratio $\hat{S}/S_{\mathrm{obs}}$. The raw estimates are highest for French, but the normalized ratios are much closer across languages, which shows why raw $\hat{S}$ values should not be compared without considering sample size and coverage.
...
\paragraph{Interpretation (Figure~\ref{fig:results_cross_language_median_shat}).}
The cross-language difference that is consistent across estimators is that French has the highest median $\hat{S}$, followed by German and then English. This is not only because French has a larger corpus: the English corpus is larger than the German one and yet scores lower, suggesting that corpus size alone does not drive the ordering.
...
\paragraph{Interpretation (Figure~\ref{fig:results_coverage_standardized_language_comparison}).}
When comparing the normalized ratios rather than the raw estimates, the French corpus no longer stands out as clearly. This is mainly because the number of estimated word types does not grow linearly with the number of tokens but rather logarithmically. The ratio plot is therefore more informative for comparing estimator behavior than the raw $\hat{S}$ plot. It shows that the amount of extrapolation relative to observed vocabulary is broadly similar across languages for a given estimator, while estimator choice still has a large effect.
...
\paragraph{Interpretation (Table~\ref{tab:results_estimator_summary_global}).}
The global summary shows a clear ordering of estimator aggressiveness. Jackknife1 gives the smallest median ratio, Chao1 and Jackknife2 give moderate ratios, iChao1 is slightly above Chao1, ACE gives larger estimates because it corrects for heterogeneity among rare types, and Breakaway gives the largest median ratio. The very large mean ratio for Breakaway indicates that a small number of successful fits produce extremely large estimates.
...
\paragraph{Interpretation (Table~\ref{tab:results_estimator_failure_summary}).}
