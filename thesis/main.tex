...
\textit{Possible addition.} The frequency spectrum shows the typical long-tailed structure of word-frequency data. A large number of word types occur only once or twice, while progressively fewer types occur at higher frequencies. This is important because most richness estimators rely heavily on the low-frequency counts, especially $f_1$ and $f_2$, when estimating how many word types remain unseen.
...
\paragraph{Interpretation.}
We can see that the from the given estimators the ACE estimator is the most aggressive one, it gives the highest estimation of vocabulary richness of Shakespeare. And the Jackknife1 estimator gives the shortes.

\textit{Possible addition.} The classical lower-bound estimators give totals between roughly 35,000 and 41,500 types, while ACE gives a substantially larger estimate of about 70,900 types. This difference reflects the fact that ACE includes an explicit heterogeneity correction for the rare part of the frequency spectrum. Breakaway is not reported for the full Shakespeare corpus because the fitted extrapolation parameter was negative, making the estimated number of unseen types undefined.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../results/figures/shakespeare_estimator_comparison.png}
    \caption{Estimated vocabulary richness for the Shakespeare replication baseline, comparing the estimators.}
    \label{fig:results_shakespeare_estimator_comparison}
\end{figure}

\paragraph{Interpretation.}
Breakaway has the highest Media $\hat{S}$ across all plays. It makes use of more frequency counts.

\textit{Possible addition.} This figure is based on the per-play Shakespeare analysis rather than the full-corpus table above. It therefore answers a slightly different question: how the estimators behave on individual Shakespeare units. The main pattern is that the ranking of estimators is similar, with Jackknife1 remaining conservative and Breakaway producing the largest median estimate among the cases where it is defined.
...
\paragraph{Interpretation.}
This Graph shows that when extrapolating the number of tokens sampled we follow a log-like function. The Chao1 estimator gives an estimation of roughly 7000 distinct types.

\textit{Possible addition.} The curve rises quickly at small sample sizes and then begins to flatten, but it has not fully saturated at the observed sample size. This means that additional sampling would still be expected to reveal new word types, although at a slower rate. The extrapolated part should be read as a model-based projection rather than as an observed quantity.
...
\begin{table}[htbp]
    ...
    \end{tabular}}
    \caption{This is a language-level overview of the spoken corpora used in the cross-language comparison. We make sure to exclude Shakespeare since his written books where used for the analysis.}
    \label{tab:results_language_corpus_overview}
\end{table}

\paragraph{Interpretation.}
This table shows that there is an imbalance in the number of tokens per language. France has the highest one, then followed by english and then German. Even though English has a higher number of tokens, its mean coverage is smaller then the one of the German one by approximately 6 percent.

\textit{Possible addition.} The table also shows why raw cross-language comparisons are difficult. French has by far the largest total number of tokens, while German has very few units. Therefore, later comparisons should use normalized quantities, such as $\hat{S}/S_{\mathrm{obs}}$ and coverage, instead of relying only on raw estimated richness.
...
\begin{table}[htbp]
    ...
    \end{tabular}}
    \caption{We show a corpus-level overview and Shakespeare is shown for context as the written replication baseline.}
    \label{tab:results_corpus_overview}
\end{table}

\paragraph{Interpretation.}
The French corpus dominates across all corpuses since it has the highest number of tokens and we are only working with one french corpus. 

\textit{Possible addition.} At the corpus level, CLAPI dominates the total amount of data, while IMSDb contributes most of the English units. The table also shows that some corpora contain very small units, which can make richness estimation unstable. Shakespeare is included only for context and should not be interpreted as part of the spoken-language comparison.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../results/figures/tokens_by_language.png}
    \caption{Distribution of token counts by language for the spoken corpora. By unit we refer to one speaker file in the processed database and we use the log scale since token counts varu a lot.}
    \label{fig:results_tokens_by_language}
\end{figure}

\paragraph{Interpretation.}
In the first plot we have the density on the y-axis since all languages dont have the same number of tokens and the french one would dominate the graph. In this way we can compare the three languages equally. The plot on the right depicts the tokens per unit for each Corpus and we can clearly see that CLAPI has the highest one.

\textit{Possible addition.} The log scale is useful because the number of tokens per unit varies over several orders of magnitude. The density view compares the shape of each language's token-count distribution, while the boxplot shows how the distributions differ by corpus. This confirms that sample size is a major confounder for all later richness comparisons.
...
\paragraph{Interpretation.}
The French language corpora has the highest sample coverage, this implies that it has the highest completeness. This implies that it has the lowest amount of word appearing only once.  
...
\textit{Possible addition.} The singleton proportions support the coverage interpretation. French has the smallest singleton share relative to observed vocabulary, while German has the largest. This helps explain why German requires more extrapolation despite having a smaller total corpus.

\textit{Possible addition.} Higher coverage means that a smaller fraction of the next sampled tokens is expected to belong to previously unseen word types. Thus, lower coverage corpora require stronger extrapolation, while higher coverage corpora are closer to being complete with respect to their observed sampling frame.
...
\paragraph{Interpretation.}
We can see that Breakaway fails quite often since it doesnt have enough consecutive nonzero frequency counts and a stable ratio-regression fit. If the fitted parameter is negative it will fail. We can see that when comparing the estimators estimations normalized they all give a very similar results across different languages. (SHow the numbers). Also as shown in the Figure 10 we can see the plots.

\textit{Possible addition.} Across all three languages, Jackknife1 is the most conservative estimator, Chao1 and Jackknife2 are intermediate, ACE is more aggressive, and Breakaway has the largest median ratio among the successful fits. The normalized ratios are fairly similar across languages: for example, Chao1 gives median ratios around 2.1--2.35, ACE around 3.8--4.1, and Breakaway around 4.9--5.3. This suggests that estimator choice affects the result more strongly than language alone.
...
SHould I inlcude the following{
\begin{figure}[htbp]
    \centering
...
\textit{Possible addition.} For English, lower coverage is associated with larger $\hat{S}/S_{\mathrm{obs}}$ ratios. This is expected because low coverage means that many word types are still likely to be unseen, so the estimators add a larger unseen-vocabulary component.
...
\textit{Possible addition.} The French corpus generally has higher coverage, but the same qualitative pattern remains: estimates become more conservative as coverage increases. Breakaway should be interpreted cautiously because many French units do not produce a stable fit.
...
\textit{Possible addition.} For German, the number of units is much smaller than for English and French, so the pattern is less stable. Nevertheless, lower-coverage units tend to have larger extrapolation ratios, which is consistent with the interpretation of coverage as sample completeness.
}
...
\paragraph{Interpretation.}
When fitting finite-k monote models, the data sticks with small/moderate k instead of pushing toward very large k. And at no point did the model kep choosing the maximum tested k. This meand that the CM behavior is only an additional model for comparison for this data and will not necesseraly be chosen as the best one.

\textit{Possible addition.} The selected values of $k^\ast$ suggest that the data need more flexibility than very low-order monotonicity, but they do not force the model all the way to the largest tested $k$. Therefore, the completely monotone model is useful as a limiting comparison, but the BIC selection does not by itself prove that complete monotonicity is required.
...
\paragraph{Interpretation.}
The selected values are moderate rather than minimal: $k^\ast=5$ for English, $k^\ast=6$ for French, $k^\ast=4$ for German, and $k^\ast=8$ for Shakespeare. This suggests that the data are not well described by very low-order monotonicity, but the optimum also does not occur at the largest tested value. Thus, the figure supports the use of moderately flexible finite-$k$ monotone models, while giving only limited evidence for treating the completely monotone model as a necessary limiting case.

\textit{Possible addition.} The steep decrease in BIC for small values of $k$ shows that increasing the order of monotonicity initially improves the fit substantially. After the selected value, the curve becomes almost flat, so additional complexity gives little improvement and is penalized by BIC.
...
\paragraph{Interpretation.}
The comparison between the selected finite-$k^\ast$ monotone estimates and the completely monotone estimates shows almost perfect agreement. Nearly all points lie on the diagonal line and it means that both approaches produce the same estimated richness for the sampled units. This suggests that, at least for these data, the completely monotone approximation does not substantially change the estimated vocabulary size relative to the BIC-selected finite-$k$ model. If the points had deviated strongly from the diagonal, this would have indicated that the choice between finite-$k$ monotonicity and complete monotonicity materially affects the richness estimates.

\textit{Possible addition.} This agreement is important because it means that, even when BIC selects finite values of $k$, the final richness estimates are almost unchanged by using the completely monotone approximation. In practice, the modelling choice affects the theoretical interpretation more than the numerical estimate in these sampled units.
...
\paragraph{Interpretation.}
We can clearly see that all curves are not close to saturation and grow with the sample size. On top of that all of them aim towards the same expected number.

\textit{Possible addition.} The curves show that additional tokens would still reveal new word types in each language. However, the representative-unit comparison should not be interpreted as proving that the languages have the same vocabulary richness, because each panel is based on one selected unit and the corpora differ strongly in size and composition.
...
\paragraph{Interpretation.}
TODO: Explain the cross-language table while being careful about sample-size and corpus-composition differences. Isnt this the same table as before?

\textit{Possible addition.} This table overlaps with the per-language estimator summary, but it is organized specifically for cross-language comparison. The important columns here are the median observed vocabulary, the median estimated total richness, the median unseen component $\hat{f}_0$, and the normalized ratio $\hat{S}/S_{\mathrm{obs}}$. The raw estimates are highest for French, but the normalized ratios are much closer across languages, which shows why raw $\hat{S}$ values should not be compared without considering sample size and coverage.
...
\paragraph{Interpretation.}
The cross-language differences that appear constant across different estimators is that the French Median $\hat{S}$ scores higher then German and English. This isnt only due because French has a bigger corpora since the English corpora is larger then the German one and still scores lower then it.

\textit{Possible addition.} A cautious interpretation is that French units have larger median estimated richness in this dataset, but this should not be read as an inherent property of the French language. It may also reflect the structure of CLAPI, the larger median token count, speaker aggregation, or differences in transcription and preprocessing.
...
\paragraph{Interpretation.}
When comparing the ratios and not the raw estimates we can see how it affects the results. The French corpora doesnt score as high anymore. This is mainly due to the fact that the number of estimated words doesnt grow linearly with the number of tokens but rather logarithmically.

\textit{Possible addition.} The ratio plot is therefore more useful for comparing estimator behavior than the raw $\hat{S}$ plot. It shows that the amount of extrapolation relative to observed vocabulary is broadly similar across languages for a given estimator, while estimator choice still has a large effect.
...
\textit{Possible addition.} The global summary shows a clear ordering of estimator aggressiveness. Jackknife1 gives the smallest median ratio, Chao1 and Jackknife2 give moderate ratios, iChao1 is slightly above Chao1, ACE gives larger estimates because it corrects for heterogeneity among rare types, and Breakaway gives the largest median ratio. The very large mean ratio for Breakaway indicates that a small number of successful fits produce extremely large estimates.
...
\textit{Possible addition.} Chao1, iChao1, and Jackknife1 are numerically stable on all units. ACE and Jackknife2 fail only rarely. Breakaway fails for about half of the units, meaning its summaries are based only on the subset of corpora where the ratio-regression fit is possible and valid. This makes Breakaway less reliable as a general-purpose estimator for the present data.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{../results/figures/estimator_correlation_heatmap.png}
    \caption{Correlation between estimator outputs across the spoken corpora.}
    \label{fig:results_estimator_correlation_heatmap}
\end{figure}

\paragraph{Interpretation.}
Although the classical estimators are almost perfectly correlated, this should not be interpreted as evidence that they give the same estimates. The high correlations mainly reflect the shared dependence on sample size and observed vocabulary size. Therefore, agreement is better assessed using ratios or differences between estimators.

\textit{Possible addition.} Because the heatmap adds limited information beyond confirming this shared scaling behavior, it could be moved to an appendix. The ratio-based comparisons below are more informative for the main results.
...
\textit{Possible addition.} This raw difference plot is dominated by extreme Breakaway values, which stretch the y-axis and make the differences among the other estimators difficult to see. The main useful message is that Breakaway can produce very large deviations from Chao1. For comparing the remaining estimators, the ratio plot is clearer.
